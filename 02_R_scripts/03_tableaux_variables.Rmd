...
# Nutrition
## Modèles linéaires simples Anemie - variables
```{r}
# Anemia nutrition
...
## Modèles linéaires simples Polyglobulie - variables
```{r}
...
# Vos variables binaires
binary_vars <- c(
  "Polyglobulie_Hb14.5_OMS2024"
  , "Polyglobulie_Hb14.5_OMS2011"
)
...
## Modèles linéaires simples QUANTITATIF - variables
```{r}
...
## Modèles linéaires simples QUALITATIF - variables
```{r}
vars_expliquees <- c(
  "kCalHors.fibres",
  "Pr..g.",
...
param_cols <- c("Anemie_OMS2011","Anemie_OMS2024", "Anemie_OMS2011_FER", "Anemie_OMS2024_FER", "Polyglobulie_Hb14.5_OMS2011", "Polyglobulie_Hb14.5_OMS2024")
...
# Tableaux divers
## Tableau variables différences entre villes
```{r}
...
signif_res <- data_all %>%
  ...
  group_modify(~{
    df <- .x
    # Exiger au moins 2 obs par ville pour éviter les erreurs
    ok <- df %>% group_by(Ville_eval) %>% summarise(n = sum(!is.na(Valeur)), .groups="drop") %>% filter(n >= 2)
    if (nrow(ok) < 2) return(tibble(comparison = NA_character_, sig = ""))
    rstatix::pairwise_t_test(df, Valeur ~ Ville_eval, p.adjust.method = "BH", pool.sd = FALSE) %>%
      mutate(
  ...
  ungroup() %>%
  mutate(comparison = stringr::str_replace_all(comparison, c(
    "Lima \\(150m\\)" = "L", "Cusco \\(3400m\\)" = "C",
    "Juliaca \\(3800m\\)" = "J", "La Rinconada \\(5100m\\)" = "R"
  ))) %>%
  group_by(Variable) %>%
...
## Tableau modalités variables qualitatives
```{r}
...
vars_quali <- c(
  ...
  "Polyglobulie_Hb1SD",
  "Polyglobulie_Hb2SD",
  "Polyglobulie_1SD_Hbmass.g.kg",
  "Polyglobulie_2SD_Hbmass.g.kg",
  "Polyglobulie_HB1.5SD",
  "Polyglobulie_1.5SD_Hbmass.kg"
)
# --------------------------------
...
villes <- c("Lima (150m)", "Cusco (3400m)", "Juliaca (3800m)", "La Rinconada (5100m)")
vars_quali <- c(
  "CSP_M",        # âge exact en mois
  "CSP_P",          # taille mesurée (cm)
  "statut.poids",    # catégorie poids (OMS : maigreur, normal, surpoids…)
  "statut.taille",   # catégorie taille (retard de croissance, normal…)
  "poids.mini.kg",   # poids min observé
  "poids.max.kg",    # poids max observé
  "Taille.mini.cm",  # taille min observée
  "Taille.max.cm"    # taille max observée
)
...
# ---------- Fonction tableau qualitatif + tests pairwise ----------
make_quali_table_compact <- function(age_selected) {

  # Sous-ensemble d'âge + forcer les qualitatives en character
  df <- data_all %>%
    filter(Age_categorie_binaire == age_selected) %>%
    mutate(across(all_of(vars_quali), ~ as.character(.x)))

  # ----- Tableau descriptif (modalités listées par ville) -----
  tab_list <- lapply(vars_quali, function(var) {
    df %>%
      group_by(Variable = var, Modalité = .data[[var]], Ville_eval) %>%
      summarise(n = n(), .groups = "drop") %>%
      ...
      group_by(Variable, Ville_eval) %>%
      summarise(txt_combined = paste(txt, collapse = "\n"), .groups = "drop") %>%
      tidyr::pivot_wider(names_from = Ville_eval, values_from = txt_combined)
  })
  tab_wide <- bind_rows(tab_list) %>%
    mutate(across(all_of(villes), ~ tidyr::replace_na(.x, ""))) %>%
    select(Variable, all_of(villes))

  # ----- Tests pairwise (ville à ville) par variable -----
  abbrev <- c(
    "Lima (150m)"         = "L",
    "Cusco (3400m)"       = "C",
    "Juliaca (3800m)"     = "J",
  ...
  tests_df <- purrr::map_dfr(vars_quali, function(var) {
    d <- df %>%
      select(Ville_eval, val = all_of(var)) %>%
      filter(!is.na(val)) %>%
      mutate(val = as.factor(val)) %>%
      droplevels()

    # Villes présentes avec données
    villes_pres <- d %>% count(Ville_eval, name = "n") %>%
      filter(n > 0) %>% pull(Ville_eval) %>% as.character()

    if (length(villes_pres) < 2) {
      return(tibble(Variable = var, pair = NA_character_, p_adj = NA_real_, sig = ""))
    }

    pairs <- utils::combn(villes_pres, 2, simplify = FALSE)

    # p-values par paire
    pvals <- map_dbl(pairs, function(pr) {
      d2 <- d %>% filter(Ville_eval %in% pr) %>% droplevels()
      tbl2 <- table(d2$val, d2$Ville_eval)

      # Si tableau dégénéré (1 seule modalité observée, ou colonne vide), on renvoie NA
      if (nrow(tbl2) < 2 || any(colSums(tbl2) == 0)) return(NA_real_)

      if (any(tbl2 < 5)) {
        suppressWarnings(stats::fisher.test(tbl2, simulate.p.value = T)$p.value)
      } else {
        suppressWarnings(stats::chisq.test(tbl2, correct = FALSE)$p.value)
      }
    })
    ...
    tibble(
      Variable = var,
      pair = map_chr(pairs, ~ paste(abbrev[.x], collapse = "-")),
      p_adj = padj,
      sig = dplyr::case_when(
        is.na(padj) ~ "",
        padj < 0.001 ~ "***",
  ...
  compar <- tests_df %>%
    ...
    summarise(
      `Comparaisons significatives` = {
        idx <- which(sig != "")
        if (length(idx) == 0) "" else paste0(paste0(pair[idx], " ", sig[idx]), collapse = "; ")
      },
      .groups = "drop"
    ) %>%
    mutate(
      `Comparaisons significatives` = ifelse(
        `Comparaisons significatives` == "", "",
        paste0("Signif: ", `Comparaisons significatives`)
      )
    )

  tab_final <- left_join(tab_wide, compar, by = "Variable")

  # ----- Flextable -----
  ft <- flextable(tab_final)

  ft <- add_header_row(
    ft,
    values = c("", rep(age_selected, length(villes)), "Comparaisons"),
    colwidths = c(1, rep(1, length(villes)), 1)
  )

  header_labels <- c("Variable" = "Variable")
  header_labels[villes] <- villes
  header_labels["Comparaisons significatives"] <- "Comparaisons significatives"
  ft <- set_header_labels(ft, .labels = header_labels)
...
print(ppt, target = "tableau_quali_modalites_ligne_par_age_avec_signif.pptx")
```
```{r}
vars_anemie <- c(
  "Anemie_OMS2011",
...
vars_polyglobulie <- c(
  ...
  "Polyglobulie_HbSD",
  "Polyglobulie_Hb2SD",
  "Polyglobulie_HbSD_Hbmass_kg",
  "Polyglobulie_Hb2SD_Hbmass_kg",
  "Polyglobulie_Hb1.5SD",
  "Polyglobulie_Hb1.5SD_Hbmass_kg"
)
vars_anthropo <- c(
  "Age_mois",        # âge exact en mois
  "Taille",          # taille mesurée (cm)
  "Poids",           # poids mesuré (kg)
  "BMI",             # indice de masse corporelle (variante du nom)
  "IMC",             # indice de masse corporelle (doublon dans ton fichier)
  "statut.poids",    # catégorie poids (OMS : maigreur, normal, surpoids…)
  "statut.taille",   # catégorie taille (retard de croissance, normal…)
  "poids.mini.kg",   # poids min observé
  "poids.max.kg",    # poids max observé
