...
for (nom_groupe in names(groupes_vars_categoriel)) {
  ...
  for (categorie_age in ages) {
    ...
    # Plot
    n_colors <- length(unique(data_plot$Valeur))

# Palette qualitative étendue basée sur Set1
palette_csp <- colorRampPalette(RColorBrewer::brewer.pal(9, "Set1"))(n_colors)

p <- ggplot(data_plot, aes(x = Ville_eval, y = Prop, fill = Valeur)) +
  geom_bar(stat = "identity", position = "dodge", width = 0.7) +
  facet_wrap(~Variable, scales = "free_y", ncol = 3) +
  geom_text(
    data = distinct(data_plot, Variable, p.signif),
    aes(label = paste0("p: ", p.signif)),
    x = 1.5, y = 1.05, inherit.aes = FALSE, size = 4, hjust = 0
  ) +
  scale_y_continuous(labels = scales::percent_format()) +
  scale_fill_manual(values = palette_csp) +   # ✅ palette adaptée automatiquement
  labs(
    title = paste("Barplots (", nom_groupe, " – ", categorie_age, ")"),
    x = "City", y = "Percentage", fill = "CSP"
  ) +
  theme_minimal(base_size = 14) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
    dml_plot <- dml(ggobj = p)
