...
data_long$Méthode <- factor(data_long$Méthode, levels = c("Anemie_OMS2011", "Anemie_SD", "Anemie_2SD", "Anemie_OMS2024","Anemie_1.5SD"))
...
data$Ville_eval <- factor(data$Ville_eval,
  levels = c("Lima", "Cusco", "Juliaca", "Rinconada"),
  labels = c("Lima (150m)", "Cusco (3400m)", "Juliaca (3800m)", "La Rinconada (5100m)")
)
...
ordre_methodes_raw <- c("Anemie_OMS2011", "Anemie_OMS2024", "Anemie_SD", "Anemie_2SD", "Anemie_SD_Hbmass_kg", "Anemie_2SD_Hbmass_kg")
ordre_methodes_clean <- gsub("Anemie_", "", ordre_methodes_raw)

data_long <- data_long %>%
  mutate(
    Méthode = factor(gsub("Anemie_", "", Méthode), levels = ordre_methodes_clean)
  )
...
# Tests globaux (par ville)
global_tests_ville <- data_long %>%
  group_by(Ville_eval) %>%
  summarise(
    p_global = {
...
# Comparaisons par ville entre méthodes
all_pvals_methode <- lapply(unique(data_long$Ville_eval), function(ville) {
  data_sub <- data_long %>% filter(Ville_eval == ville)
  
  lapply(method_pairs, function(pair) {
    m1 <- pair[1]
    m2 <- pair[2]
    
    sub_pair <- data_sub %>% filter(Méthode %in% pair)
    
    tbl <- table(sub_pair$Méthode, sub_pair$Résultat_bin)
...
# Annotation de significativité
p_values_methods <- all_pvals_methode %>%
  mutate(
    p.signif = case_when(
      p < 0.001 ~ "***",
      p < 0.01  ~ "**",
  ...
  left_join(global_tests_ville, by = "Ville_eval") %>%
  filter(p_global < 0.05)

# Position Y pour annotation
max_prev_ville <- prevalence %>%
  group_by(Ville_eval) %>%
  summarise(maxY = max(Prevalence))

p_values_methods <- p_values_methods %>%
  left_join(max_prev_ville, by = "Ville_eval") %>%
  group_by(Ville_eval) %>%
  mutate(y.position = maxY + 5 + 5 * row_number()) %>%
  ungroup()
p_values_methods <- p_values_methods %>%
...
ggplot(data_long, aes(x = Méthode, y = Résultat_bin * 100, fill = Méthode)) +
  ...
  facet_wrap(~ Ville_eval, scales = "free_y") +
  labs(
    title = "",
...
ordre_methodes_raw <- c("Anemie_OMS2024", "Anemie_OMS2011", "Anemie_SD", "Anemie_2SD")
ordre_methodes_clean <- gsub("Anemie_", "", ordre_methodes_raw)
...
data_3ans = data[data$Age_categorie_binaire == "0-3 ans",]
data_12ans = data[data$Age_categorie_binaire == "8-12 ans",]
data_kappa_3ans <- data_3ans %>%
  select(
    "OMS 2011" = Anemie_OMS2011,
    "OMS 2024" = Anemie_OMS2024,
    "Hb < -1 SD" = Anemie_SD,
    "Hb < -2 SD" = Anemie_2SD
)

cohen1 <- cohen.kappa(data_kappa_3ans)

data_kappa_12ans <- data_12ans %>%
  select(
    "OMS 2011" = Anemie_OMS2011,
    "OMS 2024" = Anemie_OMS2024,
    "Hb < -1 SD" = Anemie_SD,
    "Hb < -2 SD" = Anemie_2SD,
    "Hbmass < -1 SD" = Anemie_SD_Hbmass_kg,
    "Hbmass < -2 SD" = Anemie_2SD_Hbmass_kg
)

cohen3 <- cohen.kappa(data_kappa_12ans)
```

```{r}
pheatmap(cohen3$cohen.kappa, display_numbers = T, cluster_rows = FALSE,
cluster_cols = FALSE)
```
...
# Créer un titre dynamique
titre <- paste0(
  "Proportions croisées anémie : OMS 2024 v.s. OMS 2011\n",
  "n = ", n_total, " ; ", "pval ", methode, " = ", round(pval, 3)
)
...
# Créer un titre dynamique
titre <- paste0(
  "Proportions croisées polyglobulie : OMS 2024 v.s. OMS 2011\n",
  "n = ", n_total, " ; ", "pval ", methode, " ~ ", round(pval, 3)
)
