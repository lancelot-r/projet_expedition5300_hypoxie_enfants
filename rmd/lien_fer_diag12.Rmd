...
mod1 = randomForest(Fer..mg.100.g. ~ ., data = data_subset_all[,-2], importance = TRUE, ntree = 100000)
mod2 = randomForest(kCalHors.fibres ~ ., data = data_subset_all[,-1], importance = TRUE, ntree = 100000)
...
data <- data[data$Age_categorie_binaire == "8-12 ans",]
data_subset <- data %>%
  select(HB, HCT.., Pr..g., L.g., G.g., Fibres, Calcium.....mg., Fer..mg.100.g., Zinc..mg.100.g., Vitamine.C..mg., Ville_eval.x)
data_subset_all <- na.omit(data_subset)
data_subset_all = data_subset_all[data_subset_all$Ville_eval.x == "Rinconada",]

mod1 = lm(HB ~ ., data = data_subset_all[,-c(2, 11)])
mod2 = lm(HCT.. ~ ., data = data_subset_all[,-c(1, 11)])
