...
get_signif_logo <- function(df, var, group = "Ville_eval") {
  symbole_map <- setNames(c("C","J","L",""), sort(unique(data$Ville_eval)))

  
  df2 <- df %>% select(all_of(c(group, var))) %>% filter(!is.na(.data[[var]]))
  villes_presentes <- sort(unique(df2[[group]]))
  
  if (length(villes_presentes) < 2) {
    return(setNames(rep("", length(villes)), villes))
  }
  ...
  mutate(group1 = sub("-.*", "", comparison),
      ...
      gtest <- tryCatch(kruskal_test(df2, as.formula(paste(var, "~", group))), error = function(e) NULL)
      if (is.null(gtest) || gtest$p > 0.05) return(logos)
      posthoc <- tryCatch(
  ...
  # ====== Cas qualitatif ======
    ...
    gtest <- tryCatch(chisq_test(df2, as.formula(paste(var, "~", group))), error = function(e) NULL)
    if (is.null(gtest) || gtest$p > 0.05) return(logos)
    posthoc <- tryCatch(
...
df_diag <- data %>%
  left_join(seuils, by = c("Ville_eval", "Categorie_age_OMS")) %>%
  mutate(
    diagnostic_1SD = case_when(
      HB < seuil_1SD_inf ~ "Anémie 1SD",
      HB > seuil_1SD_sup ~ "Polyglobulie 1SD",
      TRUE ~ ""
    ),
    diagnostic_2SD = case_when(
      HB < seuil_2SD_inf ~ "Anémie 2SD",
      HB > seuil_2SD_sup ~ "Polyglobulie 2SD",
      TRUE ~ ""
    ),
    diagnostic_1SD_K = case_when(
      Hb.mass.kg..g...kg. < seuil_1SD_inf_K ~ "Anémie 1SD",
      Hb.mass.kg..g...kg. > seuil_1SD_sup_K ~ "Polyglobulie 1SD",
      TRUE ~ ""
    ),
    diagnostic_2SD_K = case_when(
      Hb.mass.kg..g...kg. < seuil_2SD_inf_K ~ "Anémie 2SD",
      Hb.mass.kg..g...kg. > seuil_2SD_sup_K ~ "Polyglobulie 2SD",
      TRUE ~ ""
    ),
    diagnostic_OMS2011 = case_when(
      HB_corrigée_OMS2011 < 11 & Categorie_age_OMS == "6-23 mois"~ "Anémie OMS2011",
      HB_corrigée_OMS2011 < 11 & Categorie_age_OMS == "24-59 mois"~ "Anémie OMS2011",
      HB_corrigée_OMS2011 < 11.5 & Categorie_age_OMS == "5-11 ans"~ "Anémie OMS2011",
      HB_corrigée_OMS2011 < 12 & Categorie_age_OMS == "12-14 ans"~ "Anémie OMS2011",
      HB_corrigée_OMS2011 > 14.5 ~ "Polyglobulie OMS2011",
      TRUE ~ ""
    ),
    diagnostic_OMS2024 = case_when(
      HB_corrigée_OMS2024 < 10.5 & Categorie_age_OMS == "6-23 mois"~ "Anémie OMS2024",
      HB_corrigée_OMS2024 < 11 & Categorie_age_OMS == "24-59 mois"~ "Anémie OMS2024",
      HB_corrigée_OMS2024 < 11.5 & Categorie_age_OMS == "5-11 ans"~ "Anémie OMS2024",
      HB_corrigée_OMS2024 < 12 & Categorie_age_OMS == "12-14 ans"~ "Anémie OMS2024",
      HB_corrigée_OMS2024 > 14.5 ~ "Polyglobulie OMS2024",
      TRUE ~ ""
    )
  )%>%
  select(Code_Sujet, Ville_eval, Age_mois, Age_categorie_binaire, Categorie_age_OMS, HB, diagnostic_1SD, Anemie_SD, Polyglobulie_SD, diagnostic_2SD, Anemie_2SD, Polyglobulie_2SD, diagnostic_OMS2011, Anemie_OMS2011, Polyglobulie_14.5_OMS2011, diagnostic_OMS2024, Anemie_OMS2024, Polyglobulie_14.5_OMS2024, diagnostic_1SD_K, Anemie_SD_Hbmass_kg,Polyglobulie_SD_Hbmass_kg, diagnostic_2SD_K, Anemie_2SD_Hbmass_kg, Polyglobulie_2SD_Hbmass_kg)
